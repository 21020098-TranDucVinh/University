...
int main()
{
    ...
    while (pcom < stacks)
    {
        ...
        // Looking for the shortest job in the process
        if (j > 0)
        {
            for (j = 1; j <= stacks; j++)
            {
    ...
    for (i = 1; i <= stacks; i++)
    {
        ...
        process[i].wat = process[i].ct - process[i].art; // Calculating the total waiting time
        process[i].ta = process[i].wat + process[i].btt; // Calculating the total turnaround time
    ...
    cout << "\nThe Preemptive SJF Scheduling Has Been Successfully Processed!" << endl
